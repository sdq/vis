...
let template = function (eventProxy) {
    let template = {},
        size = [500, 500],
        margin = {
            left: 10,
    ...
    template.data = function (_) {
        if (!arguments.length) return data;
        data = _[0] || [];
        return template;
    };

    template.dimensions = function (_) {
    ...
    template.slots = function () {
        return slots;
    };

    template.mapping = function (_) {
    ...
    template.properties = function(_) {
    ...
    template.isReady = function () {
        // TODO: check mapping ready or not
        
        return false;
    };

    template.isRendered = function() {
        return isRendered;
    }
    ...
    template.save = function() {
        ...
        return {
            //TODO: properties to save
        }
    };

    template.clear = function (json) {
        if (container) {
            container.html('');
        }
    ...
    template.operate = function (json) {
        // process actionJson
    ...
    let saveState = function (note) {
        ...
        let parameters = {
            'note': note,
            'svg': svg,
            'mapping': JSON.parse(vis.map2json(mappings)),
            'state': {
                // TODO: state to save
            }
        }
